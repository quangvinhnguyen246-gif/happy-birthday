...
let w = (c.width = window.innerWidth),
    ...
    (opts = {
        //text
        strings: ["HAPPY", "BIRTHDAY", "KIM NHI"],
        charSize: 30,
        charSpacing: 35,
        lineHeight: 40,
...
function Letter(char, x, y) {
    ...
    var originalHue = (x / calc.totalWidth) * 360;
    var hue;
    if (originalHue < 40) {
        hue = originalHue; // Keep reds (0–40)
    } else if (originalHue < 90) {
        hue = (originalHue + 140) % 360; // Shift yellows (40–70) and some oranges to blues/purples
    } else if (originalHue < 150) {
        hue = (originalHue + 120) % 360; // Shift greens (90–150) to blues/purples
    } else {
        hue = originalHue; // Keep blues (180–270) and purples (270–360)
    }
...
function anim() {
    ...
    if (done) {
        // document.querySelector('.birthday-cake').classList.add('visible');
        for (var l = 0; l < letters.length; ++l) 
            letters[l].reset();
    }
}
